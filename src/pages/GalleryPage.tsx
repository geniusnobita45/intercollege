...
export default function GalleryPage() {
  ...
  const [activePhoto, setActivePhoto] = useState<{ src: string; fallback: string; alt: string; category: string } | null>(null);
            ...
            {filteredItems.map((photo, idx) => (
              <FadeUp key={`${photo.src}-${idx}`} delayMs={(idx % 6) * 60}>
                <div
                  ...
                  className="group relative cursor-pointer overflow-hidden rounded-[1.75rem] border border-white/80 bg-white/95 shadow-md backdrop-blur-md transition-all duration-300 hover:-translate-y-1.5 hover:shadow-xl aspect-[4/3]"
                >
                  <img
                    src={photo.src}
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100 flex flex-col justify-end p-6">
                    <span className="inline-block w-fit rounded-full bg-primary/90 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-white mb-2">
                      {photo.category}
                    </span>
                    <p className="text-sm font-semibold text-white leading-snug">{photo.alt}</p>
        ...
        >
          ...
          >
            ...
            <div className="p-4 text-center">
              <span className="inline-block rounded-full bg-orange-50 px-3 py-1 text-xs font-bold uppercase text-primary mb-2">
